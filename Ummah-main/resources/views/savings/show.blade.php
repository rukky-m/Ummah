        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
            <div>
                <div class="flex items-center gap-3 mb-1">
                    <a href="{{ route('savings.index') }}" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition text-sm font-bold flex items-center gap-1">
                        <i class="fas fa-arrow-left text-xs"></i> Savings
                    </a>
                ...
                <h2 class="font-black text-2xl text-gray-900 dark:text-white leading-tight tracking-tight">
                    Savings Transaction Review
                </h2>
                <p class="text-sm text-gray-500 mt-1">
                    Processed on {{ $saving->transaction_date->format('l, d F Y') }}
                    &bull; Reference #{{ $saving->id }}
                </p>
            </div>

            <span class="px-4 py-2 text-sm font-black uppercase tracking-widest rounded-2xl
                {{ $saving->status === 'approved' ? 'bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400' :
                   ($saving->status === 'rejected' ? 'bg-red-100 dark:bg-red-500/10 text-red-700 dark:text-red-400' :
                   'bg-amber-100 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400') }}">
                <i class="fas {{ $saving->status === 'approved' ? 'fa-check-circle' : ($saving->status === 'rejected' ? 'fa-times-circle' : 'fa-clock') }} mr-1.5"></i>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="relative overflow-hidden bg-white/70 dark:bg-gray-900/50 backdrop-blur-2xl shadow-sm ring-1 ring-gray-900/5 dark:ring-white/10 rounded-3xl p-6">
                        ...
                        <div class="flex items-center gap-5 relative z-10">
                            <div class="w-16 h-16 rounded-2xl bg-gradient-to-br from-emerald-500 to-green-700 flex items-center justify-center text-white font-black text-2xl uppercase shadow-lg shadow-emerald-500/20 flex-shrink-0">
                                {{ strtoupper(substr($saving->member->full_name ?? 'M', 0, 1)) }}
                            </div>
                            <div class="flex-1">
                                <p class="text-xl font-black text-gray-900 dark:text-white tracking-tight">
                                    {{ $saving->member->full_name ?? 'Unknown Member' }}
                                </p>
                                <p class="text-sm text-gray-500 dark:text-gray-400 font-medium mt-0.5">
                                    Member ID: <span class="font-black text-gray-700 dark:text-gray-300">{{ $saving->member->member_id ?? 'N/A' }}</span>
                                </p>
                                @if($saving->member?->user?->email)
                                    <p class="text-xs text-gray-400 mt-1">
                                        <i class="fas fa-envelope mr-1"></i>{{ $saving->member->user->email }}
                                    </p>
                                @endif
                            </div>
                            <div class="text-right flex-shrink-0">
                                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Amount</p>
                                <p class="text-3xl font-black {{ $saving->type === 'deposit' ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400' }}">
                    ...
                    <div class="bg-white/70 dark:bg-gray-900/50 backdrop-blur-2xl shadow-sm ring-1 ring-gray-900/5 dark:ring-white/10 rounded-3xl p-6">
                        <h3 class="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em] mb-5">Transaction Details</h3>
                        <div class="grid grid-cols-2 gap-5">
                            <div class="bg-gray-50/80 dark:bg-white/5 rounded-2xl p-4">
                                ...
                                <p class="font-black text-gray-800 dark:text-gray-200">{{ $saving->transaction_date->format('d M Y') }}</p>
                            </div>
                            <div class="bg-gray-50/80 dark:bg-white/5 rounded-2xl p-4">
                                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Category</p>
                                <p class="font-black text-gray-800 dark:text-gray-200">{{ $saving->category ?? 'Savings' }}</p>
                            </div>
                            <div class="bg-gray-50/80 dark:bg-white/5 rounded-2xl p-4">
                                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Type</p>
                                <p class="font-black text-gray-800 dark:text-gray-200 capitalize">{{ $saving->type ?? 'Deposit' }}</p>
                            </div>
                            <div class="bg-gray-50/80 dark:bg-white/5 rounded-2xl p-4">
                                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Created On</p>
                                <p class="font-black text-gray-800 dark:text-gray-200">{{ $saving->created_at->format('d M Y, H:i') }}</p>
                            </div>
                        </div>
                    ...
                    {{-- Withdrawal Bank Details (If applicable) --}}
                        ...
                        <div class="bg-white/70 dark:bg-gray-900/50 backdrop-blur-2xl shadow-sm ring-1 ring-gray-900/5 dark:ring-white/10 rounded-3xl p-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div class="bg-gray-50/80 dark:bg-white/5 rounded-2xl p-4">
                                    ...
                                    <p class="font-black text-gray-800 dark:text-gray-200">{{ $saving->bank_name ?? 'N/A' }}</p>
                                </div>
                                <div class="bg-gray-50/80 dark:bg-white/5 rounded-2xl p-4">
                                    <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Account Number</p>
                                    <p class="font-black text-gray-800 dark:text-gray-200 tracking-widest">{{ $saving->account_number ?? 'N/A' }}</p>
                                </div>
                                <div class="bg-gray-50/80 dark:bg-white/5 rounded-2xl p-4 md:col-span-2">
                                    <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Account Name</p>
                                    <p class="font-black text-gray-800 dark:text-gray-200">{{ $saving->account_name ?? 'N/A' }}</p>
                                </div>
                            </div>
                        </div>
                    @endif

                    {{-- Payment Proof / Receipt (For deposits) --}}
                    @if($saving->type === 'deposit')
                        <div class="bg-white/70 dark:bg-gray-900/50 backdrop-blur-2xl shadow-sm ring-1 ring-gray-900/5 dark:ring-white/10 rounded-3xl p-6">
                            ...
                            @if($saving->payment_proof_path)
                                @php
                                    $ext = strtolower(pathinfo($saving->payment_proof_path, PATHINFO_EXTENSION));
                                ...
                                @if($isImage)
                                    <div class="rounded-2xl overflow-hidden ring-1 ring-gray-900/5 dark:ring-white/10 bg-gray-50 dark:bg-gray-800 relative group">
                                        <img
                                            src="{{ asset('storage/' . $saving->payment_proof_path) }}"
                                            alt="Payment Proof"
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white/70 dark:bg-gray-900/50 backdrop-blur-2xl shadow-sm ring-1 ring-gray-900/5 dark:ring-white/10 rounded-3xl p-6">
                        ...
                        <div class="space-y-3">
                            <div class="flex justify-between items-center">
                                ...
                                <span class="text-xs font-black text-gray-800 dark:text-gray-200">{{ $saving->member->full_name ?? '—' }}</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-xs font-bold text-gray-500 dark:text-gray-400">Amount</span>
                                <span class="text-sm font-black {{ $saving->type === 'deposit' ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400' }}">
                                    ₦{{ number_format($saving->amount, 2) }}
                                </span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-xs font-bold text-gray-500 dark:text-gray-400">Date</span>
                                <span class="text-xs font-black text-gray-800 dark:text-gray-200">{{ $saving->transaction_date->format('d M Y') }}</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-xs font-bold text-gray-500 dark:text-gray-400">Status</span>
                                <span class="text-xs font-black uppercase
                                    {{ $saving->status === 'approved' ? 'text-emerald-600' : ($saving->status === 'rejected' ? 'text-red-600' : 'text-amber-600') }}">
                                    {{ $saving->status }}
                                </span>
                            </div>
                            @if($saving->recordedBy ?? null)
                                <div class="flex justify-between items-center pt-3 border-t border-gray-100 dark:border-white/5">
                                    ...
                                    <span class="text-xs font-black text-gray-800 dark:text-gray-200">{{ $saving->recordedBy->name ?? '—' }}</span>
                                </div>
                            @endif
                        </div>
                    </div>
                    ...
                    {{-- Action Panel --}}
                    @if($saving->status === 'pending' && auth()->user()->canManageSavings())
                        <div x-data="{ action: 'approve' }" class="bg-white/70 dark:bg-gray-900/50 backdrop-blur-2xl shadow-sm ring-1 ring-gray-900/5 dark:ring-white/10 rounded-3xl p-6">
                            <h3 class="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em] mb-5">Admin Verification</h3>
                            ...
                            <form x-show="action === 'approve'" x-cloak
                                  action="{{ route('savings.approve', $saving) }}"
                                  method="POST" class="space-y-4">
                                @csrf
                                <div class="{{ $saving->type === 'deposit' ? 'bg-emerald-50 dark:bg-emerald-500/10 ring-emerald-500/20 text-emerald-700 dark:text-emerald-400' : 'bg-rose-50 dark:bg-rose-500/10 ring-rose-500/20 text-rose-700 dark:text-rose-400' }} rounded-2xl p-4 ring-1 text-center">
                                    <i class="fas {{ $saving->type === 'deposit' ? 'fa-check-double' : 'fa-hand-holding-usd' }} text-2xl mb-2 block"></i>
                                    <p class="text-xs font-bold leading-relaxed">
                                        @if($saving->type === 'deposit')
                                            Confirming this deposit will add ₦{{ number_format($saving->amount, 2) }} to the member's savings balance.
                                        @else
                                            Approving this withdrawal confirms that ₦{{ number_format($saving->amount, 2) }} has been transferred to the member's bank account.
                                        @endif
                                    </p>
                                </div>
                                <button type="submit"
                                        onclick="return confirm('Are you sure you want to approve this transition?')"
                                    ...
                                    <i class="fas fa-check-circle"></i> Confirm and Authorize
                                </button>
                            </form>

                            {{-- Reject Form --}}
                            <form x-show="action === 'reject'" x-cloak
                                  action="{{ route('savings.reject', $saving) }}"
                                  method="POST" class="space-y-4">
                                @csrf
                                <div>
                                    ...
                                    <textarea name="reason" rows="4" required
                                              ...
                                              placeholder="State clearly why this transaction is being rejected..."></textarea>
                    ...
                    @elseif($saving->status === 'approved')
                        <div class="bg-emerald-50 dark:bg-emerald-500/10 backdrop-blur-2xl shadow-sm ring-1 ring-emerald-500/20 rounded-3xl p-6 text-center">
                            ...
                            <p class="text-xs text-emerald-600/70 dark:text-emerald-500 mt-1">This transaction has been verified and completed.</p>
                        </div>
                    ...
                    @elseif($saving->status === 'rejected')
                        <div class="bg-red-50 dark:bg-red-500/10 backdrop-blur-2xl shadow-sm ring-1 ring-red-500/20 rounded-3xl p-6 text-center">
                            ...
                            <p class="text-xs text-red-600/70 dark:text-red-500 mt-1">This transaction was not approved.</p>
                        </div>
                    @endif
                    ...
                    <a href="{{ route('savings.index') }}"
                       class="flex items-center justify-center gap-2 w-full py-3 bg-white/70 dark:bg-gray-900/50 backdrop-blur-2xl ring-1 ring-gray-900/5 dark:ring-white/10 rounded-2xl text-xs font-black text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:ring-gray-300 dark:hover:ring-white/20 transition-all">
                        <i class="fas fa-arrow-left"></i> Back to All Savings
                    </a>
